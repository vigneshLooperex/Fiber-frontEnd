  ...
  const list = (
    <List
      style={{
        backgroundColor: "#fff",
        width: 300,
        height: user?.notifications.length >= 3 ? 300 : "auto",
        overflow: "scroll",
        overflowX: "hidden",
      }}
      ...
      >
        <div className="disktop-menu">
          {nav.map((item, i) => (
            <Link
              ...
              to={item.href}
              className={
                pathname === item.href ? "nav-links nav-active" : "nav-links"
              }
            >
              <item.icon />
              {item.title}
        ...
        <div className="desktop-right-icons">
          <Dropdown overlay={list} trigger={["click"]}>
            <Link to="" onClick={() => mute({})} className="nav-links">
              <Badge dot={user?.notify}>
          ...
          <Link to="" onClick={() => logOut()} className="nav-links">
            <LogoutOutlined style={{ fontSize: 22 }} /> LogOut
          </Link>
        </div>
      </Anchor>
